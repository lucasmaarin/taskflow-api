import React, { useState } from 'react';
import { useProjects } from '../../contexts/ProjectContext.jsx';
import { useTasks } from '../../contexts/TaskContext.jsx';

const ProjectList = () => {
  const { projects, loading, deleteProject } = useProjects();
  const { tasks } = useTasks();
  const [confirmId, setConfirmId] = useState(null);

  const getProjectTaskCount = (projectId) =>
    tasks.filter((t) => t.projectId === projectId && !t.completed).length;

  const handleDeleteClick = (e, projectId) => {
    e.stopPropagation();
    setConfirmId(projectId);
  };

  const handleConfirmDelete = async (e) => {
    e.stopPropagation();
    await deleteProject(confirmId);
    setConfirmId(null);
  };

  if (loading) return null;

  return (
    <div className="project-list-widget">
      <h3 className="widget-title">Projetos</h3>
      <ul className="project-list">
        {projects.map((project) => {
          const count = getProjectTaskCount(project.id);
          const isConfirming = confirmId === project.id;

          return (
            <li key={project.id} className={`project-list-item ${isConfirming ? 'project-confirming' : ''}`}>
              <span
                className="project-color-dot"
                style={{ backgroundColor: project.color }}
              />
              <span className="project-emoji">{project.emoji}</span>
              <span className="project-name">{project.name}</span>
              {count > 0 && (
                <span className="project-task-count">{count}</span>
              )}

              <div className="project-item-actions">
                {isConfirming ? (
                  <>
                    <button
                      className="project-action-btn confirm-delete-btn"
                      onClick={handleConfirmDelete}
                      title="Confirmar exclusão"
                    >
                      Excluir
                    </button>
                    <button
                      className="project-action-btn cancel-btn"
                      onClick={(e) => { e.stopPropagation(); setConfirmId(null); }}
                      title="Cancelar"
                    >
                      ✕
                    </button>
                  </>
                ) : (
                  <button
                    className="project-action-btn project-delete-btn"
                    onClick={(e) => handleDeleteClick(e, project.id)}
                    title="Excluir projeto"
                  >
                    <svg viewBox="0 0 20 20" fill="currentColor" width="13" height="13">
                      <path fillRule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clipRule="evenodd" />
                    </svg>
                  </button>
                )}
              </div>
            </li>
          );
        })}
      </ul>
    </div>
  );
};

export default ProjectList;
